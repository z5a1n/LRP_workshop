rm(list=ls())
library(ggplot2)
L <- read.csv("1/Ex1_Landings.csv")
D <- read.csv("1/Ex1_Data.csv")
...
#SWNS/BoF PS Catch
ggplot(D[!(is.na(D$PS_Catch)),],aes(y=PS_Catch/1000,x=Year)) + geom_path() +
  theme_classic() + labs(x='Year', y="Purse Seine Landings (t)") + expand_limits(y=0)

#SWNS/BoF PS Effort
ggplot(D[!(is.na(D$PS_Effort)),],aes(y=PS_Effort,x=Year)) + geom_path() +
